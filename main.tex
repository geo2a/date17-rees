...
system. Specifications, represented via mathematical models, can indeed be verified and
simulated, targeting an error-free system. Workcraft provides the support for different
models, this can help the design for resiliency. Conditional partial order graphs (CPOG)
...
are shown. The development of a domain specific language (DSL), that can be used for the
specification of the instructions, still remains a gap of this flow. Having a scalable, yet
...
The CPOG and Dataflow Structures (DFS) were used together to design an asynchronous
reconfigurable pipeline for the computation of the ordinal pattern encoding. CPOG was used
...
hardware description (VHDL). The DFS model matches the HDL description, the asynchronous 4-
hase dual rail protocol is implemented. Figure \ref{fig:voltage-resiliency} shows the
resiliency of the chip to the voltage variation. Workcraft, and the tools that the framework
comes with, have been already validated for the design of resilient systems, the flow can
still be improved by adopting DSL for formalising system specifications.
